window.addEventListener('DOMContentLoaded',()=>{
  /** @type {html element} variable to store the name of the HTML element where pokemon name is loaded*/
  const pokeP=document.getElementById('pokeInfo')
  /** @type {string} name of the div placeholder where pokemon informations are loaded*/
  const pokeDiv=document.getElementById('pokemon-info')
  /** @type {html element} name of the button to listen to, to create new ability on user's click*/
  const pokeAbilityBtn=document.getElementById('ability')
  
  /**
   *Async function to find pokemon in pokeapi and insert text with the name of a pokemon in HTML page if a pokemon is found
   *
   @param {number} pokedexNum get random number to get random pokemon in api 
   @param {string} foundPokemon name of the pokemon
   @param {string} jsonPokemon  will be used to store pokemon informations to create an object pokemon
  ...
  /**
   * Asyn function to find pokemon abilities and insert it as text in HTML if an ability is found
   * 
   * @param {number} pokedexNum get random number to get random pokemon ability in api 
   ...
   *
   * @param {string} pokeBtn string of the button name to listen to for user's click
   */
